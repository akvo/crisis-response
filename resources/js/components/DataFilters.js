...
class DataFilters extends Component {
    ...
    render() {
        let property =
            this.props.value.filters[this.props.value.page.name].filters;
        let base = this.props.value.base;
        let filters = uniqBy(base.data, property.base).map(
            (x) => x[property.base]
        );
        filters = base[property.kind].filter((x) => filters.includes(x.id));
        let all = {
            id: 0,
            text: "All " + titleCase(property.dropdown),
            name: "All " + titleCase(property.dropdown),
        };
        if (property.id !== 0) {
            filters = [all, ...filters];
        }
